...
Pr1 = Function(W)  ###Pr FunctionSpace created
...
Cscale = Constant(4)  ### cosh scale (I ended up just inputting 4)
Pr1.assign(Pr).at((0.5,0.5))  ###Assign Pr to const.  Later I reassign to the cosh func to output
...
F = (
    inner(cosh(4*T-4), inner(grad(u), grad(v)))*dx
    + inner(cosh(4*T-4), inner(dot(grad(u), u), v))*dx#3
    - Pr*inner(p, div(v))*dx
    - (Ra)*inner(T*g, v)*dx
    + Pr*inner(div(u), q)*dx
    + inner(cosh(4*T-4), inner(dot(grad(T), u), S))*dx
...
Pr1.rename("Prandtl") ## I named my temp-dependent Plandtl output "Prando"



File("RBProj4.pvd").write(u, p, T, Pr1) #Output Pr1 too
